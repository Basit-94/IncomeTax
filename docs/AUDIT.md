...
- current primary-source mappings for modeled AY 2026–27 values in `docs/scale/rules-audit.md`, with scope gaps still called out;
- a Spring Boot/Java 21 backend foundation with integer-paise `Money`, versioned rules resources, an append-only fact-ledger contract, a tested JDBC/PostgreSQL adapter, an asynchronous idempotent submission boundary, and 72 TypeScript-to-Java golden vectors. The isolated Maven run passes all 6 backend tests;
- an owned local load harness whose 20-request smoke passed with 20/20 journeys, 0 correctness failures, 2/2 duplicate checks, 77.27 logical RPS, p50 8.83 ms, and p95/p99 229.21 ms; bounded process-count, overload, 60-second soak, and process-loss chaos results are now recorded under `docs/scale/`. These are one-laptop/local-process observations, not production or national-capacity claims;
- a root `README.md` with setup, verification, harness commands, evidence links, and the synthetic-only boundary.

The current tree therefore addresses the historical engine/testing and presentation defects, but it is not yet a legally complete tax engine, production backend, or critic-converged product. The browser UI remains local-only; the additive backend and load harness do not contact official systems. Round-three B/C critics close the fresh correction, reload, and regime-path evidence gaps, but participant-based reuse/adoption evidence remains blocked.
