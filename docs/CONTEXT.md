...
| `/api/transcribe` | `app/api/transcribe/route.ts` → `lib/server/transcriber.ts` → `scripts/transcribe_worker.py` | **Dictation (2026-09-07):** POST multipart `audio` (≤ 8 MB) + optional `language` (a Whisper code) → `{ ok, text, language }`. Session required. Runs **faster-whisper on this machine**: one Python worker per server, spawned on first use and kept warm (JSON lines over stdin/stdout), model `WAPSI_WHISPER_MODEL` (default `small`), CUDA float16 only if a probe decode succeeds, else CPU int8; the clip lives in a temp file for the duration of the request. 503 `transcriber_unavailable` when Python/faster-whisper is missing. The client side is `lib/speech.ts`: MediaRecorder captures one phrase (auto-stop after 1.6 s of silence once speech was heard, 45 s cap; tap the mic again to stop), then posts it; `WHISPER_LANGUAGE` maps the 23 languages (15 by code, 8 to detection). The browser's SpeechRecognition is no longer used. |
