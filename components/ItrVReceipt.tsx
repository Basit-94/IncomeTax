...
export function ItrVReceipt({ filedOn, filedAt }: ItrVReceiptProps = {}) {
  ...
  return (
    <div className="mx-auto my-8 max-w-3xl space-y-4">
      ...
      */}
      <div dir="ltr" className="flex items-center justify-between rounded-xl border border-line bg-paper-3 p-4 print:hidden">
        <div>
          <h3 className="text-sm font-bold text-ink-2">Acknowledgement preview</h3>
        ...
        <button
          ...
          className="flex cursor-pointer items-center gap-2 rounded-lg ink-surface px-5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:ink-surface"
        >
      ...
      <div
        ref={receiptRef}
        dir="ltr"
        className="printable-sheet rounded-xl border-2 border-line bg-paper-3 p-8 font-sans text-ink-2 shadow-md print:rounded-none print:border-none print:p-0 print:shadow-none"
      >
        {/* This banner prints. It is the first thing on the sheet by design. */}
        <p className="mb-4 rounded border border-money/40 bg-amber-bg px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-amber-ink">
        ...
        <div className="flex items-start justify-between border-b-2 border-line pb-4">
          <div>
            ...
            <h1 className="mt-0.5 text-xl font-extrabold text-ink-2">
          ...
          <div className="text-right">
            <div className="inline-block rounded border border-dashed border-line bg-paper-3 p-2 text-center">
              <span className="block text-[10px] font-mono uppercase text-ink-3">
                e-Filing ack no
              </span>
              <span className="text-xs font-mono font-bold tabular-nums text-ink-2">
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-line py-4 text-xs">
          <div>
            <p className="text-ink-3">Name of assessee:</p>
            <p className="text-sm font-bold uppercase text-ink-2">{state.name}</p>
            <p className="mt-2 text-ink-3">PAN:</p>
            <p className="font-mono text-sm font-bold text-ink-2">{state.pan}</p>
            <p className="mt-2 text-ink-3">Status:</p>
            <p className="text-sm font-bold text-ink-2">{state.filingStatus}</p>
          </div>
          <div>
            <p className="text-ink-3">Filed under section:</p>
            <p className="font-bold text-ink-2">{sectionLabel}</p>
            <p className="mt-2 text-ink-3">Submission timestamp:</p>
            <p className="font-bold text-ink-2 tabular-nums" data-testid="itrv-timestamp">
              {submissionStamp}
            </p>
            <p className="mt-2 text-ink-3">Regime opted:</p>
        ...
        <div className="border-b border-line py-4">
          <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-ink-2">
            Statement of computation (rupees only)
          </h4>
          <table className="w-full text-xs">
            <tbody className="divide-y divide-line">
              <tr>
                <td className="py-1.5 text-ink-2">1. Gross total income</td>
                <td className="py-1.5 text-right">
                  <Rupees value={active.grossTotalIncome} className="font-semibold" />
                </td>
              </tr>
              <tr>
                <td className="py-1.5 text-ink-2">2. Standard deduction u/s 16(ia)</td>
                <td className="py-1.5 text-right">
              ...
              <tr>
                <td className="py-1.5 text-ink-2">3. Deductions under Chapter VI-A</td>
                <td className="py-1.5 text-right">
                  <span className="font-semibold text-ok-ink">
                    −
              ...
              <tr className="bg-paper-3 font-bold">
                <td className="py-2 text-ink-2">4. Total taxable income (1 − 2 − 3)</td>
                <td className="py-2 text-right">
                  <Rupees value={active.taxableIncome} className="text-ink-2" />
                </td>
              </tr>
              <tr>
                <td className="py-1.5 text-ink-2">5. Tax on total income</td>
                <td className="py-1.5 text-right">
              ...
              {active.specialRateTax > 0 && (
                <tr>
                  <td className="py-1.5 pl-4 text-ink-3">
                    of which special rates (s.111A / 112A / 112)
                  </td>
                  <td className="py-1.5 text-right">
                    <Rupees value={active.specialRateTax} className="text-ink-3" />
                  </td>
                </tr>
              )}
              {active.specialExemptAmount > 0 && (
                <tr>
                  ...
                  <td className="py-1.5 text-right">
                    <Rupees value={active.specialExemptAmount} className="text-ink-3" />
                  </td>
                </tr>
              )}
              <tr>
              ...
              <tr>
                <td className="py-1.5 text-ink-2">7. Health &amp; education cess (4%)</td>
                <td className="py-1.5 text-right">
                  <Rupees value={active.cess} className="font-semibold" />
                </td>
              </tr>
              <tr className="border-t border-line font-bold">
                <td className="py-2 text-ink-2">8. Net tax liability</td>
                <td className="py-2 text-right">
                  <Rupees value={active.totalTaxLiability} className="text-ink-2" />
                </td>
              </tr>
              <tr>
                <td className="py-1.5 text-ink-2">9. Taxes paid — TDS per 26AS</td>
                <td className="py-1.5 text-right">
                  <Rupees value={active.tdsPaid} className="font-semibold text-ok-ink" />
                </td>
              </tr>
              {active.advanceTaxPaid > 0 && (
                <tr>
                  <td className="py-1.5 text-ink-2">10. Advance tax paid</td>
                  <td className="py-1.5 text-right">
                    <Rupees
                      value={active.advanceTaxPaid}
                      className="font-semibold text-ok-ink"
                    />
                  </td>
                </tr>
              )}
              ...
              <tr className="bg-amber-bg text-sm font-extrabold text-ink">
                <td className="p-2.5">
        ...
        <div className="flex items-end justify-between gap-6 pt-4 text-[11px] text-ink-3">
          <div className="min-w-0 space-y-1">
            <p className="font-semibold text-ink-2">Verification digest:</p>
            <p className="break-all font-mono text-[9px] leading-relaxed text-ink-3">
          ...
          <div className="shrink-0 text-center">
            {verifyUrl && (
              <div className="inline-block rounded border border-line bg-paper-3 p-1.5">
                <QRCodeSVG value={verifyUrl} size={92} level="M" />
              </div>
            )}
            <p className="mt-1 max-w-[110px] text-[9px] leading-tight text-ink-3">
